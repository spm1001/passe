...
1. **Canonical markdown probe**: pages advertising `<link rel="alternate" type="text/markdown">` (Mintlify-hosted docs — code.claude.com, mintlify.com — and the spreading llms.txt convention) are served from their `.md` sibling directly; the source file beats any HTML extraction. Unadvertised siblings (platform.claude.com serves `.md` but doesn't advertise) are caught by a guessed `URL.md` probe on docs-shaped URLs (`docs.` host or `/docs/` path) and on escalation paths (SPA shell, empty extraction, quality-gate failure). A 7-day host cache (`~/.passe/md-hosts.json`) keeps the speculative GET off `.md`-less docs sites (MDN, docs.python.org) after first contact; healthy non-docs fetches never pay anything. Summary shows `source: markdown_probe`.
