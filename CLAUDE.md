...
- `ax-tree [--compact] [--depth N] [--flat-refs]` — accessibility tree as JSON. `--flat-refs` emits interactive elements only, one line each (`{"ref": "e1", "role": "link", "name": "FAQ"}`), and caches `ref → element` per tab (`~/.passe/refs/`) — then `click e1`, `type e3 "text"`, `hover e2` act by ref, **including in a later passe invocation** (scout with `--keep-tab`, act with `--reuse-tab`). Refs are cleared on navigation; a stale/unknown ref fails with "re-run ax-tree --flat-refs", not a CDP error.
