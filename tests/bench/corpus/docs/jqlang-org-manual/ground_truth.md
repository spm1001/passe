...
. When the name is not a valid identifier, this is
...
global variable. If you run jq with `--rawfile foo bar`
...
produces `1.7976931348623157e+308`
...
, precision will be lost if the exponent
is too large.
...
The examples below use the builtin function `have_decnum`

in
order to demonstrate the expected effects of using / not
using the `--disable-decnum`

build configuration option, and
...
The simplest *useful* filter has the form `.foo`

. When given a
JSON object (aka dictionary or hash) as input, `.foo`
...
syntax only works for simple, identifier-like keys, that
...
above is a shorthand version of this, but
...
When the index value is an integer, `.[<number>]`

can index
arrays. Arrays are zero-based, so `.[2]`
...
syntax can be used to return a
subarray of an array or substring of a string. The array
returned by `.[10:15]`

will be of length 5, containing the
...
outputs produced by the right. For instance, filter ```
.foo,
.bar
```

, produces both the "foo" fields and "bar" fields as
separate outputs.
...
is the input value at the particular stage
in a "pipeline", specifically: where the `.`
...
in the
middle refers to whatever value `.a`
...
of values (as in `[.foo, .bar, .baz]`

) or to "collect" all the
...
, producing every value. This is the
same as the zero-argument `recurse`

builtin (see below). This
is intended to resemble the XPath `//`
...
to find all the values of object keys
...
**string**is the number of Unicode codepoints it contains (which will be the same as its JSON-encoded length in bytes if it's pure ASCII). -
The length of a
...
**object**is the number of key-value pairs. -
The length of
...
The builtin function `utf8bytelength`
...
In the absence of errors, `map(f)`

always outputs an array
whereas `map_values(f)`

outputs an array if given an array,
or an object if given an object.

When the input to `map_values(f)`

is an object, the output
object has the same keys as the input object except for
those keys whose values when piped to `f`

produce no values
at all.

The key difference between `map(f)`
...
Here are some examples to clarify the behavior of `map`

and
`map_values`

when applied to arrays. These examples assume the
input is `[1]`
...
specified sequence of path expressions, such that if `p`

is any
one of these specifications, then `(. | p)`

will evaluate to the
same value as `(. | pick(pathexps) | p)`
...
must be an array of paths, where each path is an array
...
does the opposite conversion, and `with_entries(f)`

is a shorthand for `to_entries | map(f) | from_entries`
...
as raw output (i.e., strings will not
...
operates on the given generator rather than
the input.
...
function parses its input as a boolean. It
...
is identical to `recurse(f; true)`
...
function applies f recursively to every
...
is an object representing the environment variables as
set when the jq program started.

`env`

outputs an object representing jq's current environment.
...
, applies percent-decoding, by mapping
all `%XX`
...
, input is decoded as specified by RFC 4648.
...
representation consumed by `mktime`
...
if any value in the source stream
...
is irrelevant. If you're coming from JavaScript, please note that jq's `==`
...
branch is optional, and if absent is the
same as `.`

. This also applies to `elif`

with absent ending `else`
...
; you'll
need something like `if .name == "" then A else B end`
...
rather than with special syntax, as in ```
.foo and .bar |
not
```
...
operator produces all the values of its left-hand
side that are neither `false`

nor `null`

. If the
left-hand side produces no values other than `false`
...
produces all the values of its right-hand
side.

A filter of the form `a // b`

produces all the results of
`a`
...
produces no
results, or no results other than `false`
...
. The latter will
produce default values for all non-`false`
...
, which always produces just one value,
while in `(false, null, 1) // 42`
...
signifying a whitespace character would be written as `"\\s"`
...
- Ignore empty matches`p`

- Both s and m modes are enabled`s`
...
- Find longest possible matches`x`
...
- offset in UTF-8 codepoints from the beginning of the input`length`

- length in UTF-8 codepoints of the match`string`

- the string that it matched`captures`
...
- length in UTF-8 codepoints of this capturing group`string`

- the string that was captured`name`

- the name of the capturing group (or`null`
...
. If the regex contains capturing
...
We use a variable, `$names`

, to store the realnames object, so that we
...
which is visible "to the right" of it, ```
... | .*3 as
$times_three | [. + $times_three] | ...
```

, but not "to the
left". Consider this expression now, ```
... | (.*3 as
$times_three | [. + $times_three]) | ...
```
...
. C math functions that take three input
...
with the null input option `-n`

to prevent one input from being read
implicitly.

Two builtins provide minimal output capabilities, `debug`

, and
`stderr`

. (Recall that a jq program's output values are always
...
Note that when using `input`

it is generally necessary to
invoke jq with the `-n`

command-line option, otherwise
...
inputs. Note that when using `inputs`

it is generally necessary
to invoke jq with the `-n`
...
These two filters are like `.`

but have as a side-effect the
production of one or more messages on stderr.

The message produced by the `debug`
...
is a compact rendition of the input
value. This format may change in the future.

The `debug(msgs)`

filter is defined as `(msgs | debug | empty), .`
...
The examples below use the streamed form of `["a",["b"]]`
...
will not get
bigger, even if you've previously set `.bar = .foo`
...
by using a reduction to compute new, replacement values for `.`

that
have had all the desired assignments applied to `.`
...
, sees the original value, not the modified
value.
...
If the right-hand side outputs no values (i.e., `empty`
...
produces multiple values, then for each such
value jq will set the paths on the left-hand side to the value
and then it will output the modified `.`
...
. The "update" assignment forms (see above) do
not do this.
...
The former will set the `a`

field of the input to the `b`

field of the input, and produce the output `{"a": 20, "b": 20}`

.
The latter will set the `a`

field of the input to the `a`
...
A dependency with relative path `foo/bar`
...
exists in the user's home directory, and is a file (not a
...
directory in a search path. A `.jq`
...
should be an object with keys like `homepage`
...
directory in a search path. A `.json`
...
an object with keys like `homepage`
...
as an array value for the `deps`
...
line is considered a comment by jq, so it is ignored. But it is not ignored by`sh`

, since in`sh`

a backslash at the end of the line does not continue the comment. With this trick, when the script is invoked as`total 1 2`

,`/bin/sh -- /path/to/total 1 2`

will be run, and`sh`

will then run`exec jq --args -MRnf -- /path/to/total 1 2`

replacing itself with a`jq`
