...
## Introduction[¶](https://go.dev#introduction)
...
It augments the [language specification](https://go.dev/ref/spec),
the [Tour of Go](https://go.dev/tour/),
and [How to Write Go Code](https://go.dev/doc/code.html),
all of which you
should read first.
...
See [issue
28782](https://go.dev/issue/28782) for context.

### Examples[¶](https://go.dev#examples)

The [Go package sources](https://go.dev/src/)
...
[go.dev](https://go.dev/) web site, such as
[this one](https://go.dev/pkg/strings/#example-Map) (if
...
## Formatting[¶](https://go.dev#formatting)
...
emits them by default. Use spaces only if you must. - Line length
...
## Commentary[¶](https://go.dev#commentary)
...
## Names[¶](https://go.dev#names)

Names are as important in Go as in any other language. They even have semantic effect: the visibility of a name outside a package is determined by whether its first character is upper case. It's therefore worth spending a little time talking about naming conventions in Go programs.

### Package names[¶](https://go.dev#package-names)
...
### Interface names[¶](https://go.dev#interface-names)
...
### MixedCaps[¶](https://go.dev#mixed-caps)
...
## Semicolons[¶](https://go.dev#semicolons)
...
## Control structures[¶](https://go.dev#control-structures)
...
### Redeclaration and reassignment[¶](https://go.dev#redeclaration)
...
The blank identifier has many uses, as described in [a later section](https://go.dev#blank).
...
See [the language specification](https://go.dev/ref/spec#Rune_literals)
for details.)
The loop
...
### Type switch[¶](https://go.dev#type_switch)
...
## Functions[¶](https://go.dev#functions)

### Multiple return values[¶](https://go.dev#multiple-returns)
...
### Named result parameters[¶](https://go.dev#named-results)
...
## Data[¶](https://go.dev#data)

### Allocation with `new`

[¶](https://go.dev#allocation_new)
...
### Constructors and composite literals[¶](https://go.dev#composite_literals)
...
### Two-dimensional slices[¶](https://go.dev#two_dimensional_slices)
...
you can use the [blank identifier](https://go.dev#blank) (`_`
...
### Printing[¶](https://go.dev#printing)
...
See the section below on [pointers vs. value receivers](https://go.dev#pointers_vs_values) for more information.)
...
In the [initialization section](https://go.dev#initialization) we'll see another technique that avoids this recursion.
...
## Initialization[¶](https://go.dev#initialization)

Although it doesn't look superficially very different from initialization in C or C++, initialization in Go is more powerful. Complex structures can be built during initialization and the ordering issues among initialized objects, even among different packages, are handled correctly.

### Constants[¶](https://go.dev#constants)
...
## Methods[¶](https://go.dev#methods)

### Pointers vs. Values[¶](https://go.dev#pointers_vs_values)
...
## Interfaces and other types[¶](https://go.dev#interfaces_and_types)

### Interfaces[¶](https://go.dev#interfaces)
...
### Conversions[¶](https://go.dev#conversions)
...
### Interface conversions and type assertions[¶](https://go.dev#interface_conversions)

[Type switches](https://go.dev#type_switch) are a form of conversion: they take an interface and, for
...
### Generality[¶](https://go.dev#generality)
...
### Interfaces and methods[¶](https://go.dev#interface_methods)
...
## The blank identifier[¶](https://go.dev#blank)

We've mentioned the blank identifier a couple of times now, in the context of
[ for range loops](https://go.dev#for)
and


[maps](https://go.dev#maps). The blank identifier can be assigned or declared with any value of any type, with the value discarded harmlessly. It's a bit like writing to the Unix
...
### The blank identifier in multiple assignment[¶](https://go.dev#blank_assign)
...
### Unused imports and variables[¶](https://go.dev#blank_unused)
...
### Import for side effect[¶](https://go.dev#blank_import)
...
[net/http/pprof](https://go.dev/pkg/net/http/pprof/)

import _ "net/http/pprof"

This form of import makes clear that the package is being imported for its side effects, because there is no other possible use of the package: in this file, it doesn't have a name. (If it did, and we didn't use that name, the compiler would reject the program.)

### Interface checks[¶](https://go.dev#blank_implements)

As we saw in the discussion of [interfaces](https://go.dev#interfaces_and_types) above,
...
package, which defines a [encoding/json](https://go.dev/pkg/encoding/json/)

interface. When the JSON encoder receives a value that implements that interface,
the encoder invokes the value's marshaling method to convert it to JSON
instead of doing the standard conversion.
The encoder checks this property at run time with a [Marshaler](https://go.dev/pkg/encoding/json/#Marshaler)[type assertion](https://go.dev#interface_conversions) like:
...
[json.RawMessage](https://go.dev/pkg/encoding/json/#RawMessage)`json.Marshaler`
...
## Embedding[¶](https://go.dev#embedding)
...
## Concurrency[¶](https://go.dev#concurrency)

### Share by communicating[¶](https://go.dev#sharing)
...
### Goroutines[¶](https://go.dev#goroutines)
...
### Channels[¶](https://go.dev#channels)
...
See the [Go wiki](https://go.dev/wiki/LoopvarExperiment) for details.)
...
### Channels of channels[¶](https://go.dev#chan_of_chan)
...
### Parallelization[¶](https://go.dev#parallel)
...
[runtime.NumCPU](https://go.dev/pkg/runtime#NumCPU)
...
simultaneously.
It defaults to the value of [runtime.GOMAXPROCS](https://go.dev/pkg/runtime#GOMAXPROCS)`runtime.NumCPU`
...
[this
blog post](https://go.dev/blog/concurrency-is-not-parallelism).

### A leaky buffer[¶](https://go.dev#leaky_buffer)
...
statement here is another [type assertion](https://go.dev#interface_conversions).
If it fails, `ok`
...
## A web server[¶](https://go.dev#web_server)
...
If this is too quick an explanation, see the [documentation](https://go.dev/pkg/html/template/)
