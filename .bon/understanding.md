...
## The CI Matrix Lesson (July 2026)

CI runs a 3.11 + 3.14 matrix (oldest-supported + newest Python, actions pinned by SHA) because a single-version CI is structurally blind to stdlib API removals — during the 3.14 fix (passe-bovunu), the same code was simultaneously green on CI's 3.12 and red on local 3.14, and both observations were true. The failure class also arrives disguised: the 23-test breakage wasn't triggered by an interpreter upgrade but by a dependabot pytest-asyncio bump, which withdrew the implicit MainThread event loop that had been masking latent `asyncio.get_event_loop()` misuse in sync test helpers. Test-fixture behaviour is part of the dependency surface — a dep bump can withdraw an accident the code was leaning on, and the resulting CI failure on a *security* PR then blocks remediation until diagnosed. When a dependabot PR fails CI in seconds, read the PR run's own job log first (`gh run view --log`, not `--log-failed`, which can be all checkout noise): the failure is usually the bump *revealing* something, not the bump being wrong.
...
## The Launch-and-Tab Model (July 2026, passe-vicido)

Four field reports in two days (besohe/gilizu/cavudo/ketome) were one unstated model surfacing as symptoms. The decided rules, one per question every invocation answers:

1. **The address you give is the address we use.** Auto-launch derives its port from the resolved endpoint (`connect()` has no port parameter any more); `--no-first-run` keeps fresh profiles off the welcome page.
2. **If passe opens a browser for a human, it works where the human can see.** `conn_info['launched']` ('headless'|'gui'|None) records whether THIS invocation launched Chrome; a fresh-GUI launch promotes the script tab to foreground. `passe login [url]` (connect(force_visible=True)) is the human-login moment as a verb — visible Chrome, foreground tab, kept + recorded.
3. **Passe remembers its own tabs and never grabs a stranger's.** Kept tabs → `~/.passe/last-tab.json` (per endpoint, tabmemory.py). `--reuse-tab` resolves via a ladder: `--tab pattern` > cached eN refs (newest refs file among live tabs — refs are per-tab, so the act step lands where the scout snapped) > last kept tab > goto-origin match > **fail with the open-tab list** (the old first-tab grab is gone; the only path that could attach chrome:// was the empty-after-filter fallback, also gone). Policy lives in `commands._resolve_reuse_tab`; `client.attach_to_tab` is deliberately dumb.

Repro facts worth keeping: the 30s keep-on-fail flash timer NEVER worked on navigated tabs — window.close() is blocked without a script opener (armed live, tab survived) — so it was removed as a default; `--flash` stays explicit and documented as best-effort. Auto-launched *headless* Chrome dies at connect() teardown ('_process' set → cmd_run's ephemeral path suppresses the resume hint). A background-created tab reports visibilityState=hidden/hasFocus=false (matched pair vs --foreground) — the Mac "blank window" was the script running correctly in a tab the human couldn't see. GUI launch path verified live on the Mac same night (2026-07-23, Sameer watching over ssh): `passe login` opened example.com front-and-centre, and the afternoon's exact failure shape (`run --cdp` + goto on an empty port) opened with the scripted page in the foreground tab, visibilityState=visible.
