...
Repro facts worth keeping: the 30s keep-on-fail flash timer NEVER worked on navigated tabs — window.close() is blocked without a script opener (armed live, tab survived) — so it was removed as a default; `--flash` stays explicit and documented as best-effort. Auto-launched *headless* Chrome dies at connect() teardown ('_process' set → cmd_run's ephemeral path suppresses the resume hint). A background-created tab reports visibilityState=hidden/hasFocus=false (matched pair vs --foreground) — the Mac "blank window" was the script running correctly in a tab the human couldn't see. NOT yet verified: the GUI *launch* path on a real display (tube shell has none) — Mac smoke pending.
